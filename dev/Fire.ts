...
var Get:{(url:string):string} = (url:string):string => {
    var xmlHttp:XMLHttpRequest = new XMLHttpRequest();
    xmlHttp.open( "GET", url, false );
    xmlHttp.send( null );
    console.log("GET: '"+url+"'\t=>\t" + xmlHttp.statusText);
...
class Fire{
    ...
    private static InitShaders(gl:WebGLRenderingContext):void{
        if(Fire.vert == undefined){
            ...
            gl.shaderSource(Fire.vert, Get("./shaders/vert.glsl"));
        ...
        if(Fire.frag == undefined){
            ...
            gl.shaderSource(Fire.frag, Get("./shaders/frag.glsl"));
